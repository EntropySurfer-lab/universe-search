#!/usr/bin/env python3
"""
Universe Search Research Analyzer v24.1 Research Archive
...
NUMERIC_METRICS = [
    ...
    "organism_lifetime",
    "organism_survived_probe",
    "organism_collapse_tick",
    "organism_peak_objects",
    "organism_peak_largest",
    "organism_peak_defects",
    "organism_events",
    "organism_health_end",
    "organism_bonus",
...
def world_description(m: Dict[str, Any]) -> List[str]:
    ...
    qps = safe_float(m.get("quasi_particle_score")) or 0.0
    org_life = safe_float(m.get("organism_lifetime")) or 0.0
    org_peak = safe_float(m.get("organism_peak_largest")) or 0.0
    org_events = safe_float(m.get("organism_events")) or 0.0
    info_surv = safe_float(m.get("information_survival")) or 0.0
    ident = safe_float(m.get("identity_persistence")) or 0.0
    legacy = safe_float(m.get("legacy_score")) or 0.0
    post = safe_float(m.get("post_collapse_structure")) or 0.0

    if org_life >= 1500:
        notes.append("long-lived organism probe")
    elif org_life >= 500:
        notes.append("moderate organism lifetime")
    if org_peak >= 1000:
        notes.append("large organism-like structure")
    if org_events >= 6:
        notes.append("many organism lifecycle events")
    if info_surv >= 0.60:
        notes.append("strong information survival signal")
    if ident >= 0.60:
        notes.append("high identity persistence")
    if legacy >= 0.50 or post >= 0.50:
        notes.append("strong post-collapse legacy trace")
...
def build_research_summary(data: Dict[str, Any], worlds: Sequence[Dict[str, Any]], atlas_worlds: Sequence[Dict[str, Any]], corr_rows: Sequence[Tuple[str, str, float, int]], outliers: Sequence[Dict[str, Any]]) -> str:
    ...
    lines.append("- `information_analysis.md` — information and legacy metric analysis")
    lines.append("")
    return "\n".join(lines)
...
def build_information_report(worlds: Sequence[Dict[str, Any]], corr_rows: Sequence[Tuple[str, str, float, int]]) -> str:
    lines = ["# Information / Legacy Analysis", ""]
    lines.append("This report treats information cautiously: not as proof of digital life, but as a measurable trace that survives time, collapse, or structural change.")
    lines.append("")
    ...
    lines.append("## Information Metric Summary")
    lines.append("")
    rows = []
    for metric in metrics:
        s = metric_summary(worlds, metric)
        if s:
            rows.append([metric, fmt_num(s["mean"]), fmt_num(s["median"]), fmt_num(s["max"]), fmt_num(s["stdev"])])
    lines.append(make_markdown_table(["Metric", "Mean", "Median", "Max", "Std"], rows) if rows else "_No information metrics found yet._")
    lines.append("")

    lines.append("## Top Worlds by Information / Legacy")
    lines.append("")
    for metric in ["information_survival", "identity_persistence", "legacy_score", "post_collapse_structure", "organism_lifetime"]:
        leaders = top_by_metric(worlds, metric, 8)
        if not leaders:
            continue
        lines.append(f"### Top by `{metric}`")
        lines.append("")
        ...
        for w in leaders:
            ...
            rows.append([
                world_label(w),
                fmt_num(m.get(metric)),
                fmt_num(w.get("score")),
                w.get("class"),
                fmt_num(m.get("memory_trace_score")),
                fmt_num(m.get("organism_lifetime")),
            ])
        lines.append(make_markdown_table(["World", metric, "Score", "Class", "Memory", "OrgLife"], rows))
        lines.append("")

    lines.append("## Information-relevant correlations")
    lines.append("")
    info_names = set(metrics)
    rows = []
    for a, b, r, n in corr_rows:
        if a in info_names or b in info_names:
            rows.append([a, b, f"{r:+.3f}", n])
    lines.append(make_markdown_table(["Metric A", "Metric B", "Pearson r", "N"], rows[:30]) if rows else "_No information correlations available yet._")
    lines.append("")

    lines.append("## Working hypotheses")
    lines.append("")
    lines.append("- If `legacy_score` is high after collapse, the world may preserve structural information even after organism death.")
    lines.append("- If `identity_persistence` tracks `memory_trace_score`, memory may be tied to stable entity identity rather than raw activity.")
    lines.append("- If `expansion_front_speed` predicts later survival, early colonization dynamics may shape long-term organization.")
    lines.append("- If information metrics are uncorrelated with score, the current objective may be missing an important research dimension.")
    lines.append("")
    return "\n".join(lines)
...
def prepare_research_archive(out_dir: Path):
    analysis_dir = out_dir / "analysis"
    analysis_dir.mkdir(parents=True, exist_ok=True)
    nums=[]
    for p in analysis_dir.glob("experiment_*"):
        m=re.match(r"experiment_(\d+)", p.name)
        if m: nums.append(int(m.group(1)))
    exp_dir=analysis_dir / f"experiment_{max(nums,default=0)+1:04d}"
    exp_dir.mkdir()
    return exp_dir,analysis_dir,{"created":datetime.now().isoformat(timespec="seconds")}

def finalize_research_archive(exp_dir: Path, analysis_dir: Path, meta: dict):
    latest=analysis_dir/"latest"
    if latest.exists():
    ...
    (exp_dir/"experiment.json").write_text(json.dumps(meta,indent=2),encoding="utf-8")
    shutil.copy2(exp_dir/"experiment.json", latest/"experiment.json")


# ---------------- Research Archive v24.1 ----------------

def _archive_next_experiment_id(analysis_dir: Path) -> int:
    nums = []
    for p in analysis_dir.glob("experiment_*"):
        m = re.match(r"experiment_(\d+)$", p.name)
        if m:
...
def prepare_research_archive(results_dir: Path):
    """
    Creates a new non-overwriting analysis/experiment_XXXX folder and returns:
    (experiment_dir, analysis_dir, experiment_id, created_iso)
    """
    analysis_dir = results_dir / "analysis"
    analysis_dir.mkdir(parents=True, exist_ok=True)

    exp_id = _archive_next_experiment_id(analysis_dir)
    exp_dir = analysis_dir / f"experiment_{exp_id:04d}"
    exp_dir.mkdir(parents=True, exist_ok=False)

    created = datetime.now().isoformat(timespec="seconds")
    return exp_dir, analysis_dir, exp_id, created


def copy_to_latest(exp_dir: Path, analysis_dir: Path):
    """Refreshes analysis/latest with a copy of the newest experiment outputs."""
    latest = analysis_dir / "latest"
    if latest.exists():
        shutil.rmtree(latest)
...
def _archive_safe_get_best(worlds):
    if not worlds:
        return {}
    return max(worlds, key=lambda w: safe_float(w.get("score")) if safe_float(w.get("score")) is not None else -1e18)


def write_experiment_metadata(exp_dir: Path, analysis_dir: Path, exp_id: int, created: str, results_dir: Path, worlds):
    """Writes experiment.json and appends experiment_index.csv."""
    best = _archive_safe_get_best(worlds)
    ...
    meta = {
        ...
        "unique_world_records": len(worlds),
        "best_rule": best.get("rule_id") if isinstance(best, dict) else None,
        "best_generation": best.get("generation") if isinstance(best, dict) else None,
        "best_score": best.get("score") if isinstance(best, dict) else None,
        "best_class": best.get("class") if isinstance(best, dict) else None,
        "best_memory": best_metrics.get("memory_trace_score"),
        "best_tracks": best_metrics.get("persistent_tracks"),
        "best_organism_lifetime": best_metrics.get("organism_lifetime"),
        "best_information_survival": best_metrics.get("information_survival"),
        "best_identity_persistence": best_metrics.get("identity_persistence"),
        "best_legacy_score": best_metrics.get("legacy_score"),
    }
    ...
    index_file = analysis_dir / "experiment_index.csv"
    fieldnames = [
        "experiment", "created", "results_folder", "unique_world_records",
        "best_rule", "best_generation", "best_score", "best_class",
        "best_memory", "best_tracks", "best_organism_lifetime",
        "best_information_survival", "best_identity_persistence", "best_legacy_score"
    ]
...
def main() -> None:
    ...
    print("Universe Search Research Analyzer v24.1 Research Archive")
    print("=" * 42)
...
# v24.1 archive integration note: main() structure did not expose final print marker.
